 ...
 * @version 1.0
 * @date ${date}
 * Description: [${table.tableDesc}Biz服务实现]
 */
@Service
public class ${ClassName}BizServiceImpl extends CommonService implements I${ClassName}BizService {
	@Autowired
	...
	*/
	...
	public ${ClassName} select${ClassName}ById(${pkColumn.type} ${pkColumn.name}) {
		return ${className}Service.select${ClassName}ById(${pkColumn.name});
	}
	...
	*/
	...
	public int delete${ClassName}ByIds(${pkColumn.type}[] ${pkColumn.name}s) {
		return ${className}Service.delete${ClassName}ByIds(${pkColumn.name}s);
	}
	...
	*/
	...
	public int delete${ClassName}ById(${pkColumn.type} ${pkColumn.name}) {
		return ${className}Service.delete${ClassName}ById(${pkColumn.name});
	}
